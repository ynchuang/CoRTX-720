...
class BaseModel(nn.Module):
    ...
    def fit(self, x=None, y=None, batch_size=None, epochs=1, verbose=1, initial_epoch=0, validation_split=0.,
        ...
        for epoch in range(initial_epoch, epochs):
            ...
            try:
                with tqdm(enumerate(train_loader), disable=verbose != 1) as t:
                    for _, (x_train, y_train) in t:
                        ...
                        y_pred, stack_out = model(x) #.squeeze()
                        ...
                        optim.step()
                        try:
                            out_emb = torch.cat((out_emb, stack_out), 0)
                        except:
                            out_emb = stack_out
            ...
            if self.stop_training:
                break
        #print(stack_out)
        #self.embedding = stack_out
        callbacks.on_train_end()

        return self.history, out_emb
    ...
    def evaluate(self, x, y, batch_size=256):
        ...
        """
        pred_ans, _ = self.predict(x, batch_size)
        eval_result = {}
        for name, metric_fun in self.metrics.items():
    ...
    def predict(self, x, batch_size=256):
        ...
        with torch.no_grad():
            ...
            for _, x_test in enumerate(test_loader):
                x = x_test[0].to(local_device).float()
                y_pred, stack_out = model(x)#.cpu().data.numpy()  # .squeeze()
        ...
        return np.concatenate(pred_ans).astype("float64"), out_emb
